---
title: "Pain and motor severity in early Parkinson disease covary without detectable temporal precedence: a six-wave analysis of the Parkinson's Progression Markers Initiative"
...
The random intercept assumes a common rate of change. That assumption is not tenable here (Figure 8B): adding a latent slope to each construct improved fit by a large margin (likelihood ratio 39.5 on 7 degrees of freedom, p < 0.001; CFI 0.983 vs 0.960, RMSEA 0.047 vs 0.067, SRMR 0.043 vs 0.069). The improvement is not evenly shared between the two constructs. Between-person variance in rate was appreciable for motor severity (1.704, p = 0.079) and indistinguishable from zero for pain (0.006, p = 0.376), which is what a progressing motor scale and a stationary single item should look like.
...
The intraclass correlation was 0.392 for pain, against 0.535 for the motor score, 0.595 for MoCA and 0.626 for depression. Figure 10 places this alongside the eight mechanistic hypotheses, because it is the bound under which all of them were tested. Only about 0.392 of the variance in pain lies between patients.
