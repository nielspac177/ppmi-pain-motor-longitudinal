...
## The analytic strategy in one figure

![Analytic strategy: three questions, the estimator that answers each, and what it produced](outputs/paper/figures/figure1_methods.png)

Three obstacles stand between the association and a directional claim, and each
needs a different estimator. The figure pairs each question with the estimator
that answers it and the finding it produced. The causal assumptions behind each
one are drawn as directed acyclic graphs in
[`outputs/paper/figures/figure7_dags.pdf`](outputs/paper/figures/figure7_dags.pdf),
with the Mermaid sources in [`docs/dags-longitudinales.md`](docs/dags-longitudinales.md).

Every figure in the repository is generated by `R/paper/06_figuras.R` and reads
its numbers from `cifras.json`, for the same reason the text does: an earlier
version of this figure kept publishing three superseded numbers because they had
been typed into the script by hand.
