...
**Main outcomes and measures.** MDS-UPDRS Part III, examined off medication or before treatment. The primary temporal analysis was a random-intercept cross-lagged panel model, which separates stable between-person differences from within-person change; the conventional cross-lagged panel model was fitted alongside it for comparison. Attrition was handled by inverse probability of censoring weighting[@robins2000_ipcw] with a weighted generalised estimating equation, and dopaminergic dose by a marginal structural model. Specificity was assessed against depression as an alternative exposure, against cognition as an alternative outcome, and against healthy control and prodromal cohorts.

**Results.** Mean age was {{edad_media}} years (SD {{edad_de}}); {{pct_varones}} were male. A conventional cross-lagged model made both directions significant (pain to motor p = {{clpm_dolor_motor_p}}; motor to pain p = {{clpm_motor_dolor_p}}). Separating stable between-person differences fit substantially better (CFI {{fit_ri_libre_cfi}}/{{fit_clpm_cfi}}, RMSEA {{fit_ri_libre_rmsea}}/{{fit_clpm_rmsea}}, SRMR {{fit_ri_libre_srmr}}/{{fit_clpm_srmr}}) and eliminated the motor-to-pain path (p = {{riclpm_motor_dolor_p}}); the remaining pain-to-motor path survived {{refut_n_sig}} of {{refut_n_total}} robustness checks and is not claimed. What remained was a stable between-person correlation (r = {{rasgo_libre_r}}, p = {{rasgo_libre_p}}). Baseline pain marked a persistently higher motor score ({{mixto_nivel_est}} points per pain point, p = {{mixto_nivel_p}}) but not a steeper slope (p = {{mixto_pendiente_p}}), below the minimal clinically important difference and independent of striatal dopamine transporter binding ({{h2_dat_dolor_beta}}, p = {{h2_dat_dolor_p}}, against {{h2_dat_motor_beta}} for motor severity). Five converging analyses indicated the covariation is not specific to pain, including a general factor leaving no residual association (p = {{h3_anidado_p}}), an equal correlation in {{coh_ctrl_n}} healthy controls (p = {{mg_wald_p}}), and other non-motor items covarying as strongly ({{par_insomnio_r}} for insomnia against {{par_dolor_r}} for pain).
...
The reason this remains unsettled is methodological rather than substantive. Cross-sectional designs measure both quantities on the same day and cannot invoke temporal order. The longitudinal designs used so far, when they estimate lagged effects at all, use models that confound two different sources of variation. If patients differ stably in how much pain and how much motor impairment they carry, a conventional cross-lagged panel model will report significant paths in both directions purely from that stable covariation, without anyone ever moving in response to anyone else.[@Hamaker2015] Distinguishing "patients with more pain have more motor impairment" from "when a patient's pain rises, their motor score rises afterwards" requires a model that separates the two. That separation has been applied in this cohort to anxiety and depression against physical function, where no within-person coupling with the examined motor score was found although coupling with patient-reported function was,[@hodgson2026_ppmi_crosslag] and in Parkinson disease to cognition and mood,[@schroeders2022_riclpm_pd] but not to pain.
...
We also tested a specific mechanistic prediction. If the association reflects shared pathophysiology rather than confounding, pain should concentrate in the akinetic-rigid presentation and its association with motor severity should be stronger there.[@ren2020_pigd_nms_pain][@rodriguezviolante2017_kpps]
...
**Sensitivity to unmeasured confounding.** E-values[@vanderweele2017_evalue] were computed on the standardised scale, using the standard deviation of the outcome, not on the raw coefficient, a distinction that is frequently mishandled.[@vanderweele2019_evalue_correction]
...
We therefore refitted the censoring model sequentially, conditioning on remaining at the previous visit and adding that visit's pain, motor score, MoCA and dose. Those weights are more dispersed, with a standard deviation reaching {{ipcw_tv_de_max}} against {{ipcw_de_max}} for the baseline-only version, so the simpler model was indeed underfit. Their means remain close to one ({{ipcw_tv_media_min}} to {{ipcw_tv_media_max}}). Refitting the weighted model with them gives a level effect of {{tv_nivel_est}} (p = {{tv_nivel_p}}) and a slope interaction of p = {{tv_pend_p}}, so the substantive conclusions hold under either weighting. What we can say is bounded accordingly: censoring is weakly informative given the measured history, not that attrition is harmless.
...
Pain was more common and more severe in the postural instability and gait difficulty phenotype, with an adjusted ordinal odds ratio of {{fen_or_pigd}} (95% CI {{fen_or_pigd_ic}}, p = {{fen_or_pigd_p}}) relative to tremor dominance. This is consistent in direction with previous reports, though smaller in magnitude.[@ren2020_pigd_nms_pain][@rodriguezviolante2017_kpps]
...
Pain is not the largest of these. Insomnia exceeds it, and all four are of the same order. Together with the failed depression control, the depression differential matching the pain differential, the single general factor that leaves no residual association, and the equal correlation in prodromal participants and healthy controls, this is the fifth of five converging analyses pointing the same way. Two of them, the cohort comparison and the phenotype interaction, are underpowered non-rejections rather than positive evidence, and the failed depression control and the depression differential share a measure and are not independent of each other. We therefore do not present the covariation as specific to pain, while noting that the convergence is stronger than any single test in it.
...
What survives is modest, and five converging analyses agree that it is not specific to pain: the depression negative control that fails, the depression differential that matches the pain differential, a single general factor that leaves no residual pain and motor association, an equal correlation in people without the disease, and other non-motor items of the same questionnaire that covary with motor severity as strongly as pain does or more. We treat the general-burden reading as what the data support, not as an alternative we failed to exclude.
...
The minimal clinically important difference for worsening on MDS-UPDRS Part III is 4.63 points.[@horvath2015_mcid] Our level effect is {{mixto_nivel_est}} points per point of pain, so even across the full 0 to 4 range of the item it reaches about four points, and the dichotomous weighted estimate ({{msm_iptw_est}}) is roughly a third of the threshold. The trait correlation of {{rasgo_libre_r}} corresponds to about 2% of shared variance.
