...
poner("pend_r_ri", n2(pc$r_rasgo[i_ri], 3))
poner("pend_p_ri", pv2(pc$p_rasgo[i_ri]))
poner("pend_r_curva", n2(pc$r_rasgo[i_ce], 3))
poner("pend_p_curva", pv2(pc$p_rasgo[i_ce]))
poner("pend_r_basal", n2(pc$r_rasgo[i_ba], 3))
poner("pend_dm_p_ri", pv2(pc$p_dolor_motor[i_ri]))
poner("pend_dm_p_curva", pv2(pc$p_dolor_motor[i_ce]))
poner("pend_md_p_curva", pv2(pc$p_motor_dolor[i_ce]))
poner("pend_cfi_ri", n2(pc$cfi[i_ri], 3))
poner("pend_cfi_curva", n2(pc$cfi[i_ce], 3))
poner("pend_rmsea_ri", n2(pc$rmsea[i_ri], 3))
poner("pend_rmsea_curva", n2(pc$rmsea[i_ce], 3))
poner("pend_srmr_ri", n2(pc$srmr[i_ri], 3))
poner("pend_srmr_curva", n2(pc$srmr[i_ce], 3))
poner("pend_aic_ri", n2(pc$aic[i_ri], 0))
poner("pend_aic_curva", n2(pc$aic[i_ce], 0))
poner("pend_bic_ri", n2(pc$bic[i_ri], 0))
poner("pend_bic_curva", n2(pc$bic[i_ce], 0))
pl <- T("t16_pendiente_lrt.csv")
poner("pend_lrt_chi", n2(pl$chisq_dif[1], 1))
poner("pend_lrt_gl", as.character(pl$gl[1]))
poner("pend_lrt_p", pv2(pl$p[1]))
pv <- T("t16_varianza_pendientes.csv")
poner("pend_var_dolor", n2(pv$varianza[pv$pendiente == "Sd"][1], 3))
poner("pend_var_dolor_p", pv2(pv$p[pv$pendiente == "Sd"][1]))
poner("pend_var_motor", n2(pv$varianza[pv$pendiente == "Sm"][1], 3))
poner("pend_var_motor_p", pv2(pv$p[pv$pendiente == "Sm"][1]))
# El desplazamiento absoluto de la correlacion, calculado y no tecleado.
poner("pend_desplazamiento", n2(abs(pc$r_rasgo[i_ce] - pc$r_rasgo[i_ri]), 3))
write_json(C, ruta, auto_unbox = TRUE, pretty = TRUE)
cat(sprintf("  %d cifras tras la pendiente aleatoria\n", length(C)))
